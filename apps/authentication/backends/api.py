    ...
    def authenticate_credentials(request, access_key_id, request_signature):
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed(
                _('HTTP header: Date not provide or not %a, %d %b %Y %H:%M:%S GMT'))
...
class SignatureAuthentication(signature.SignatureAuthentication):
    ...
    def fetch_user_data(self, access_key_id, algorithm="hmac-sha256"):

        try:
            key = AccessKey.objects.get(access_key_id=access_key_id)
            if not key.is_active:
                return None, None
            user, secret = key.user, str(key.secret_access_key)
            return user, secret

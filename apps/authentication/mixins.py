...
from account.utils import is_block_login, clean_failed_count
from . import errors
...
class AuthMixin:
    ...
    def check_user_auth(self):
        ...
        user, error = check_user_valid(
            ...
            public_key=public_key, code=code
        )

        ip = self.get_request_ip()
        if not user:
            raise errors.CredentialError(
                username=username, error=error, ip=ip, request=self.request
            )
    ...
    def check_user_auth_if_need(self):
        ...
        if request.session.get('auth_password') and request.session.get('user_id'):
            user = self.get_user_from_session()
            if user:
                return user
        return self.check_user_auth()

    def check_user_auth_if_registered(self):
        request = self.request

...
from common.utils import common as uid
...
class AccessKey(models.Model):
    ...
    access_key_id = models.CharField(
        verbose_name='AccessKeyID', unique=True, editable=False,
        default=uid.generate_access_key, max_length=256
    )
    secret_access_key = fields.EncryptCharField(
        verbose_name='AccessKeySecret', default=uid.generate_secret_access_key,
        editable=False, max_length=1288
    )
    ...
    def get_id(self):
        return str(self.access_key_id)

    def get_secret(self):
        return str(self.secret_access_key)

    def get_full_value(self):
        return '{}:{}'.format(self.access_key_id, self.secret_access_key)
...
class LoginConfirmSetting(CommonModelMixin):
    user = models.OneToOneField(
        ...
        verbose_name=_("User"), related_name="login_confirm_setting"
    )
    reviewers = models.ManyToManyField(
        'account.User', verbose_name=_("Reviewers"),
        related_name="review_login_confirm_settings", blank=True
    )

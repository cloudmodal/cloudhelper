...
class Ticket(CommonModelMixin):
    ...
    user = models.ForeignKey(
        ...
        related_name='%(class)s_requested', verbose_name=_("User")
    )
    ...
    assignee = models.ForeignKey(
        'account.User', on_delete=models.SET_NULL, null=True,
        related_name='%(class)s_handled', verbose_name=_("Assignee")
    )
    ...
    assignees = models.ManyToManyField(
        'account.User', related_name='%(class)s_assigned',
        verbose_name=_("Assignees")
    )
    assignees_display = models.CharField(
        max_length=128, verbose_name=_("Assignees display name"),
        blank=True
    )
    type = models.CharField(
        max_length=16, choices=TYPE_CHOICES,
        default=TYPE_GENERAL, verbose_name=_("Type")
    )
...
class Comment(CommonModelMixin):
    ...
    user = models.ForeignKey(
        'account.User', on_delete=models.SET_NULL,
        null=True, verbose_name=_("User"), related_name='comments'
    )
    user_display = models.CharField(max_length=128, verbose_name=_("User display name"))
    body = models.TextField(verbose_name=_("Body"))

...
class IsValidUser(permissions.IsAuthenticated, permissions.BasePermission):
    """允许具有访问有效用户，处于活动状态且账户未过期"""

    def has_permission(self, request, view):
        return super(IsValidUser, self).has_permission(request, view) and request.user.is_valid
...
class IsAppUser(IsValidUser):
    ...
    def has_permission(self, request, view):
        return super(IsAppUser, self).has_permission(request, view) and request.user.is_app


class IsSuperUser(IsValidUser):
    def has_permission(self, request, view):
        return super(IsSuperUser, self).has_permission(request, view) and request.user.is_superuser


class IsSuperUserOrAppUser(IsSuperUser):
    def has_permission(self, request, view):
        return super(IsSuperUserOrAppUser, self).has_permission(
            request, view) or request.user.is_authenticated and request.user.is_app


class IsSuperAuditor(IsValidUser):
    def has_permission(self, request, view):
        return super(IsSuperAuditor, self).has_permission(request, view) and request.user.is_super_auditor
...
class IsOrgAuditor(IsValidUser):
    def has_permission(self, request, view):
        if not current_org:
            return False
        return super(IsOrgAuditor, self).has_permission(request, view) and current_org.can_audit_by(request.user)
...
class IsOrgAdmin(IsValidUser):
    ...
    def has_permission(self, request, view):
        if not current_org:
            return False
        return super(IsOrgAdmin, self).has_permission(request, view) and current_org.can_admin_by(request.user)
...
class IsOrgAdminOrAppUserOrUserReadonly(IsOrgAdminOrAppUser):
    def has_permission(self, request, view):
        if IsValidUser.has_permission(self, request, view) and request.method in permissions.SAFE_METHODS:
            return True
        else:

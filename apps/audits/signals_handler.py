...
def create_operate_log(action, sender, resource, **kwargs):
    ...
    data = {
        "user": str(user.username), 'action': action, 'resource_type': str(resource_type),
        'resource': str(resource), 'remote_addr': remote_addr,
    }
    with transaction.atomic():
        try:
            write_operate_logs(**data)
...
def on_object_created_or_update(sender, instance=None, created=False, **kwargs):
    if created:
        ...
        action = 'update'
    create_operate_log(action, sender, instance, **kwargs)


@receiver(post_delete, dispatch_uid="my_unique_identifier")
def on_object_delete(sender, instance=None, **kwargs):
    create_operate_log('delete', sender, instance)
